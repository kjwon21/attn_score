...
def DataProcess():
    ...
    df = pd.read_csv(data_folder + "SKT_COPA_Train.tsv", delimiter='\t')
    df.drop(['ID'], axis=1, inplace=True)
    df['question'].replace(['원인','결과'],['0', '1'], inplace=True)
    df['Answer'].replace([1, 2],[0, 1], inplace=True)

    df2 = pd.read_csv(data_folder + "SKT_COPA_Dev.tsv", delimiter='\t')
    df2.drop(['ID'], axis=1, inplace=True)
    df2['question'].replace(['원인','결과'],['0', '1'], inplace=True)
    df2['Answer'].replace([1, 2],[0, 1], inplace=True)

    sep = " [SEP] "
    train_texts, train_labels = [], []
    for _, row in df.iterrows():
        if(row['question'] == '1'):
            train_texts += [row['sentence'] + sep + row['1'], row['sentence'] + sep + row['2']]
        else:
            train_texts += [row['1'] + sep + row['sentence'], row['2'] + sep + row['sentence']]
        if(row['Answer'] == 0): 
            train_labels += [1, 0]
        else: 
            train_labels += [0, 1]
    ...
    for _, row in df2.iterrows():
        if random.random() < 0.5:
            if(row['question'] == '1'):
                val_texts += [row['sentence'] + sep + row['1'], row['sentence'] + sep + row['2']]
            else:
                val_texts += [row['1'] + sep + row['sentence'], row['2'] + sep + row['sentence']]
            if(row['Answer'] == 0): 
                val_labels += [1, 0]
            else: 
                val_labels += [0, 1]
            ...
            if(row['question'] == '1'):
                test_texts += [row['sentence'] + sep + row['1'], row['sentence'] + sep + row['2']]
            else:
                test_texts += [row['1'] + sep + row['sentence'], row['2'] + sep + row['sentence']]
            if(row['Answer'] == 0): 
                test_labels += [1, 0]
            else: 
                test_labels += [0, 1]
    ...
    val_dataset = TDataset(val_encodings, val_labels)     
    tensor_val_labels = [val_dataset[i]['labels'] for i in range(0, len(val_dataset), 2)]

    test_encodings = tokenizer(test_texts, truncation=True, padding=True)
    test_dataset = TDataset(test_encodings, test_labels)
    tensor_test_labels = [test_dataset[i]['labels'] for i in range(0, len(test_dataset), 2)]
...
def main(train_dataset, val_dataset, val_labels, test_dataset, test_labels, bf=0.0):
    ...
    def predict(trainer, dataset, labels):
        ...
        for i in range(0, len(predictions), 2):
            if(predictions[i][0] >= predictions[i][1]): tmp = 0
            else: tmp = 1
            if(predictions[i+1][0] >= predictions[i+1][1]): tmp2 = 0
            else: tmp2 = 1

            if(tmp == tmp2):
                if(tmp == 0):
                    if((predictions[i][0] - predictions[i][1]) >= (predictions[i+1][0] - predictions[i+1][1])): 
                        tmp = 0
                    else:
                        tmp = 1
                else:
                    if((predictions[i][1] - predictions[i][0]) >= (predictions[i+1][1] - predictions[i+1][0])):
                        tmp = 1
                    else:
                        tmp = 0
    ...
    print("Accuracy (CoPA) : %.2f,  (test: %.2f,)" % (e_acc, t_acc))


if __name__ == '__main__':
    parser = argparse.ArgumentParser(prog="copa_r_mm.py")
    ...
    print("copa_r_mm.py - start_bfactor=%.2f, end_bfactor=%.2f, inc_bfactor=%.2f, use_prem_grp=%s, use_lr_grp=%s, bf_prob=%.2f, prem_prob=%.2f, epoch_cnt=%d" \
