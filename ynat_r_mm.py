...
def DataProcess():
    ...
    df = pd.read_json(data_folder + 'ynat-v1.1_train.json')
    df = df[["title", "label"]]
    train_texts = df['title'].tolist()
    train_labels = df['label'].tolist()

    df2 = pd.read_json(data_folder + 'ynat-v1.1_dev.json')
    df2 = df2[["title", "label"]]
    test_texts = df2['title'].tolist()
    test_labels = df2['label'].tolist()
...
def main(train_dataset, val_dataset, val_labels, test_dataset, test_labels, bf=0.0):
    def model_init():
        if 'roberta' in model_path:  return RobertaForSequenceClassification.from_pretrained(model_path, num_labels=7, ignore_mismatched_sizes=True)
        elif 'electra' in model_path:  return ElectraForSequenceClassification.from_pretrained(model_path, num_labels=7, ignore_mismatched_sizes=True)
        else:  return BertForSequenceClassification.from_pretrained(model_path, num_labels=7, ignore_mismatched_sizes=True)
    ...
    def predict(trainer, dataset, labels):
        ...
        acc = f1_score(labels, res_labels, average='macro') * 100
        return acc
    ...
    print("Macro F1 score (YNAT) : %.2f,  (test: %.2f,)" % (e_acc, t_acc))


if __name__ == '__main__':
    parser = argparse.ArgumentParser(prog="ynat_r_mm.py")
    ...
    print("ynat_r_mm.py - start_bfactor=%.2f, end_bfactor=%.2f, inc_bfactor=%.2f, use_prem_grp=%s, use_lr_grp=%s, bf_prob=%.2f, prem_prob=%.2f, epoch_cnt=%d" \
