...
def DataProcess():
    def add_tar_token(sent, psrt, pend):
        s = sent[:pend] + tar_b + sent[pend:]
        s = s[:psrt] + tar + s[psrt:]
        return s
    ...
    df = pd.read_csv(data_folder + "NIKL_SKT_WiC_Train.tsv", delimiter='\t')
    df.drop(['ID'], axis=1, inplace=True)
    df['ANSWER'].replace([False, True], [0, 1], inplace=True)

    df2 = pd.read_csv(data_folder + "NIKL_SKT_WiC_Dev.tsv", delimiter='\t')
    df2.drop(['ID'], axis=1, inplace=True)
    df2['ANSWER'].replace([False, True], [0, 1], inplace=True)

    sep, tar, tar_b = " [SEP] ", "[TARTGET]", "[/TARGET]"
    train_texts, train_labels = [], []
    for _, row in df.iterrows():
        s1 = add_tar_token(row['SENTENCE1'], row['start_s1'], row['end_s1'])
        s2 = add_tar_token(row['SENTENCE2'], row['start_s2'], row['end_s2'])
        train_texts.append(s1 + sep + s2)
        train_texts.append(s2 + sep + s1)
        train_labels += [row['ANSWER']] * 2
    # train_labels = df['ANSWER'].values

    test_texts = []
    for _, row in df2.iterrows():
        s1 = add_tar_token(row['SENTENCE1'], row['start_s1'], row['end_s1'])
        s2 = add_tar_token(row['SENTENCE2'], row['start_s2'], row['end_s2'])
        test_texts.append(s1 + sep + s2)
    test_labels = df2['ANSWER'].values
...
def main(train_dataset, val_dataset, val_labels, test_dataset, test_labels, bf=0.0):
    ...
    print("Accuracy (WiC) : %.2f,  (test: %.2f,)" % (e_acc, t_acc))


if __name__ == '__main__':
    parser = argparse.ArgumentParser(prog="wic_q_mm.py")
    ...
    print("wic_q_mm.py - start_bfactor=%.2f, end_bfactor=%.2f, inc_bfactor=%.2f, use_prem_grp=%s, use_lr_grp=%s, bf_prob=%.2f, prem_prob=%.2f, epoch_cnt=%d" \
