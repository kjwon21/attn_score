...
class TDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        item['labels'] = torch.tensor(self.label2idx[self.labels[idx]])
        return item
...
def DataProcess():
    ...
    df = pd.read_json(data_folder + 'klue-nli-v1.1_train.json')
    df = df[["premise", "hypothesis", "gold_label"]]
    sep = " [SEP] "
    train_texts = []
    for _, row in df.iterrows():
         train_texts.append(row["premise"] + sep + row["hypothesis"])
    train_labels = df['gold_label'].tolist()

    df2 = pd.read_json(data_folder + 'klue-nli-v1.1_dev.json')
    df2 = df2[["premise", "hypothesis", "gold_label"]]
    test_texts = []
    for _, row in df2.iterrows():
         test_texts.append(row["premise"] + sep + row["hypothesis"])
    test_labels = df2['gold_label'].tolist()
...
def main(train_dataset, val_dataset, val_labels, test_dataset, test_labels, bf=0.0):
    def model_init():
        if 'roberta' in model_path:  return RobertaForSequenceClassification.from_pretrained(model_path, num_labels=3, ignore_mismatched_sizes=True)
        elif 'electra' in model_path:  return ElectraForSequenceClassification.from_pretrained(model_path, num_labels=3, ignore_mismatched_sizes=True)
        else:  return BertForSequenceClassification.from_pretrained(model_path, num_labels=3, ignore_mismatched_sizes=True)
    ...
    print("Accuracy (NLI) : %.2f,  (test: %.2f,)" % (e_acc, t_acc))


if __name__ == '__main__':
    parser = argparse.ArgumentParser(prog="nli_r_mm.py")
    ...
    print("nli_r_mm.py - start_bfactor=%.2f, end_bfactor=%.2f, inc_bfactor=%.2f, use_prem_grp=%s, use_lr_grp=%s, bf_prob=%.2f, prem_prob=%.2f, epoch_cnt=%d" \
