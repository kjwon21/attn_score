...
class BatchEncoding(UserDict):
    ...
    def convert_to_tensors(
        ...
        for key, value in self.items():
            # print('(', len(value), len(value[0]), ')', end='', flush=True)            # kjwon
            try:
...
class PreTrainedTokenizerBase(SpecialTokensMixin, PushToHubMixin):
    ...
    model_input_names: List[str] = ["input_ids", "token_type_ids", "attention_mask", "boost_mask"]      # kjwon
    ...
    def prepare_for_model(
        ...
        first_boost=None,                             # kjwon
        second_boost=None,                            # kjwon
        add_special_tokens: bool = True,
        padding: Union[bool, str, PaddingStrategy] = False,
        truncation: Union[bool, str, TruncationStrategy] = False,
        max_length: Optional[int] = None,
        ...
        **kwargs
        ...
        if truncation_strategy != TruncationStrategy.DO_NOT_TRUNCATE and max_length and total_len > max_length:
            ids, pair_ids, overflowing_tokens, first_boost, second_boost = self.truncate_sequences(     # kjwon
                ids,
                pair_ids=pair_ids,
        ...
        # kjwon
        if second_boost:
            first_boost += [0] + second_boost + [0] * (len(encoded_inputs["input_ids"]) - (len(first_boost) + len(second_boost) + 1))
        else:
            first_boost += [0] * (len(encoded_inputs["input_ids"]) - len(first_boost))           
        encoded_inputs["boost_mask"] = first_boost
        ...
        assert len(encoded_inputs["input_ids"]) == len(encoded_inputs["boost_mask"])        # kjwon
    ...
    def truncate_sequences(
        ...
        ids: List[int],
        pair_ids: Optional[List[int]] = None,
        first_boost: List[int] = None,                              # kjwon
        second_boost: List[int] = None,                             # kjwon
        num_tokens_to_remove: int = 0,
        truncation_strategy: Union[str, TruncationStrategy] = "longest_first",
        stride: int = 0,
    ) -> Tuple[List[int], List[int], List[int], List[int], List[int]]:         # kjwon
        """
        ...
        if num_tokens_to_remove <= 0:
            return ids, pair_ids, [], first_boost, second_boost         # kjwon
        ...
        if truncation_strategy == TruncationStrategy.ONLY_FIRST or (
            ...
            if len(ids) > num_tokens_to_remove:
                ...
                first_boost = first_boost[:-num_tokens_to_remove+2]     # kjwon
            else:
                error_msg = (
            ...
            for i in range(num_tokens_to_remove):
                if pair_ids is None or len(ids) > len(pair_ids):
                    ids = ids[:-1]
                    # kjwon
                    if i >= 2:
                        first_boost = first_boost[:-1]                          
                else:
                    pair_ids = pair_ids[:-1]
                    # kjwon
                    if i >= 2:
                        second_boost = second_boost[:-1]                          
        elif truncation_strategy == TruncationStrategy.ONLY_SECOND and pair_ids is not None:
            if len(pair_ids) > num_tokens_to_remove:
                window_len = min(len(pair_ids), stride + num_tokens_to_remove)
                overflowing_tokens = pair_ids[-window_len:]
                pair_ids = pair_ids[:-num_tokens_to_remove]
                second_boost = second_boost[:-num_tokens_to_remove+2]               # kjwon
            else:
                logger.error(
                    f"We need to remove {num_tokens_to_remove} to truncate the input "
        ...
        return (ids, pair_ids, overflowing_tokens, first_boost, second_boost)       # kjwon
    ...
    def _pad(
        ...
        if needs_to_be_padded:
            ...
            if self.padding_side == "right":
                ...
                encoded_inputs["boost_mask"] = encoded_inputs["boost_mask"] + [0] * difference              # kjwon
                ...
                encoded_inputs["boost_mask"] = [0] * difference + encoded_inputs["boost_mask"]              # kjwon
            else:
