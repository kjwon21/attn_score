...
class BatchEncoding(UserDict):
    ...
    def convert_to_tensors(
        ...
        for key, value in self.items():
            # kjwon
            # print('(', len(value), len(value[0]), ')', end='', flush=True)
            if key == 'boost_mask': continue
...
class PreTrainedTokenizerBase(SpecialTokensMixin, PushToHubMixin):
    ...
    def prepare_for_model(
        ...
        **kwargs
        ...
        if truncation_strategy != TruncationStrategy.DO_NOT_TRUNCATE and max_length and total_len > max_length:
            ids, pair_ids, overflowing_tokens, boost = self.truncate_sequences(             # kjwon
                ids,
                boost=boost,                                                                # kjwon
                pair_ids=pair_ids,
        ...
        # kjwon
        # boost += [0] * (len(encoded_inputs["input_ids"]) - len_boost)
        encoded_inputs["boost_mask"] = boost
    ...
    def truncate_sequences(
        self,
        ids: List[int],
        boost: List[int],                                           # kjwon
        pair_ids: Optional[List[int]] = None,
        num_tokens_to_remove: int = 0,
        truncation_strategy: Union[str, TruncationStrategy] = "longest_first",
        stride: int = 0,
    ) -> Tuple[List[int], List[int], List[int], List[int]]:         # kjwon
        """
        ...
        if num_tokens_to_remove <= 0:
            return ids, pair_ids, [], boost                     # kjwon
        ...
        if truncation_strategy == TruncationStrategy.ONLY_FIRST or (
            ...
            if len(ids) > num_tokens_to_remove:
                ...
                # boost = boost[:-num_tokens_to_remove+2]         # kjwon
            else:
                error_msg = (
        ...
        return (ids, pair_ids, overflowing_tokens, boost)       # kjwon
